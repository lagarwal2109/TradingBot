...
class EnhancedSignalGenerator:
    ...
    def compute_trading_signal(
        ...
        # 1. Trend-following breakout BUY conditions (MODERATE CONSERVATIVE)
        if (long_trend.direction == "bullish" and 
            long_trend.strength > 0.4 and  # Moderate long-term trend (40%+)
            breakout.level_type == "resistance" and
            (breakout.volume_confirmed or volume_profile.volume_ratio > 1.3) and  # Volume confirmation or 1.3x+
            volume_profile.volume_ratio > 1.2 and  # Require 1.2x+ volume
            breakout.false_breakout_risk < 0.4):  # Accept moderate false breakout risk (<40%)
            ...
            signal["reason"] = f"Bullish breakout above {breakout.level:.2f} with strong volume confirmation"
            signal["stop_loss"] = breakout.level * 0.985  # 1.5% below breakout level (TIGHT for Sortino)
            signal["take_profit"] = ticker_data["price"] * 1.08  # 8% target (ASYMMETRIC R:R ~5:1)
            signal["volume_confirmed"] = True
            signal["trend_aligned"] = True
            
        # 2. Momentum continuation BUY (MODERATE - requires good alignment)
        elif (long_trend.direction == "bullish" and
              long_trend.strength > 0.4 and  # Moderate long-term trend (40%+)
              (short_trend.direction == "bullish" or short_trend.strength > 0.4) and  # Short-term bullish or 40%+ strength
              (entry_trend.direction == "bullish" or entry_trend.strength > 0.4) and  # Entry bullish or 40%+ strength
              (volume_profile.volume_trend == "increasing" or volume_profile.volume_ratio > 1.1) and  # Volume increasing or 1.1x+
              volume_profile.volume_ratio > 1.0 and  # At least average volume
            ...
            signal["reason"] = "Strong bullish momentum with increasing volume (all timeframes aligned)"
            
            # Find nearest support for stop loss
            current_price = ticker_data["price"]
            support_below = [s for s in levels["support"] if s < current_price]
            if support_below:
                signal["stop_loss"] = max(support_below) * 0.985  # 1.5% below support (TIGHT for Sortino)
            else:
                signal["stop_loss"] = current_price * 0.985  # 1.5% stop (TIGHT for Sortino)
                
            signal["take_profit"] = current_price * 1.08  # 8% target (ASYMMETRIC R:R ~5:1)
            signal["volume_confirmed"] = volume_profile.volume_ratio > 1.5  # Require 1.5x+ volume
            signal["trend_aligned"] = True
            
        # 3. Moderate momentum BUY (requires trend alignment but not perfect)
        elif (long_trend.direction == "bullish" and
              long_trend.strength > 0.35 and  # Moderate trend strength
              short_trend.direction != "bearish" and  # Not bearish
              entry_trend.direction != "bearish" and  # Not declining
              volume_profile.volume_ratio > 1.0 and  # At least average volume
              not volume_profile.is_divergence and
              (long_trend.strength + short_trend.strength) > 0.7):  # Combined strength threshold
            ...
            signal["reason"] = "Moderate bullish momentum with trend alignment"
            
            current_price = ticker_data["price"]
            support_below = [s for s in levels["support"] if s < current_price]
            if support_below:
                signal["stop_loss"] = max(support_below) * 0.985  # 1.5% below support (TIGHT)
            else:
                signal["stop_loss"] = current_price * 0.985  # 1.5% stop (TIGHT for Sortino)
                
            signal["take_profit"] = current_price * 1.08  # 8% target (ASYMMETRIC R:R ~5:1)
            signal["volume_confirmed"] = volume_profile.volume_ratio > 1.1
            signal["trend_aligned"] = True
            
        # 5. Trend reversal SELL conditions (support breakdown) - MODERATE
        elif (breakout.level_type == "support" and
              (breakout.volume_confirmed or volume_profile.volume_ratio > 1.3) and  # Volume confirmation or 1.3x+
              volume_profile.volume_ratio > 1.2 and  # Require 1.2x+ volume
              breakout.false_breakout_risk < 0.4):  # Accept moderate false breakout risk
        ...
        # 6. Climax/exhaustion SELL - CONSERVATIVE
        elif (volume_profile.is_climax and 
              long_trend.direction == "bullish" and
              long_trend.strength > 0.6 and  # Strong uptrend
              ticker_data["price"] > prices.tail(min(100, len(prices))).mean() * 1.10):  # 10%+ above average
        ...
        # 7. Bearish momentum SELL - MODERATE
        elif (long_trend.direction == "bearish" and
              long_trend.strength > 0.4 and  # Moderate bearish trend
              (short_trend.direction == "bearish" or short_trend.strength > 0.4) and  # Short-term bearish or 40%+ strength
              entry_trend.direction != "bullish" and  # Not bullish
              volume_profile.volume_ratio > 1.1 and  # Volume confirmation
              not volume_profile.is_divergence):
        ...
        if signal["signal"] != "neutral":
            ...
            if signal["volume_confirmed"] and volume_profile.volume_ratio > 1.5:
                quality_factors.append(0.9)  # Strong volume confirmation
            elif signal["volume_confirmed"] and volume_profile.volume_ratio > 1.3:
                quality_factors.append(0.8)  # Good volume
            elif volume_profile.volume_ratio > 1.1:
                quality_factors.append(0.6)  # Above average volume
            elif volume_profile.volume_ratio > 1.0:
                quality_factors.append(0.5)  # Average volume
            else:
                quality_factors.append(0.3)  # Low volume - moderate penalty
    ...
    def rank_trading_opportunities(
        ...
        """
        # Filter for actionable signals - MODERATE CONSERVATIVE
        # Require good quality but not too strict
        actionable = [
            (pair, sig) for pair, sig in signals.items()
            if sig["signal"] != "neutral" and sig.get("entry_quality", 0) > 0.55  # Moderate threshold (55%)
        ]
