  ...
  const fetchPdf = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success' && response.data.pdf_data) {
        ...
        if (onFetchPdf) onFetchPdf();
      } else {
        setMessage(`Error: ${response.data.message || 'Please try later'}`);
      }
    } catch (error) {
      setMessage(`Error: ${error.response?.data?.message || error.message || 'Please try later'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const sendPdfEmail = async () => {
    ...
    try {
      ...
      if (response.data.status === 'success' && response.data.email_sent) {
        ...
        setMessage(`Error: ${response.data.message || 'Service is down'}`);
      }
    } catch (error) {
      setEmailStatus('error');
