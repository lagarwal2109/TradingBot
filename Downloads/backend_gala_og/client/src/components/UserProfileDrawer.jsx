...
export const UserProfileDrawer = ({ currentLanguage, setCurrentLanguage }) => {
  ...
  const API_ENDPOINT = "https://teaching.mayacode.io/api/api";
  ...
  const updatePersonaOnBackend = (personaData, isAutoSave = false) => {
    ...
    return apiService.user.updateUserPersona(dataToSend) // Use the configured API service
    .then(response => {
  ...
  const fetchUserPersonaAndLocation = () => {
    ...
    apiService.user.getUserPersona() // Use the configured API service that includes cookies
      .then(response => {
        const initialPersonaData = response.data;
  ...
  // Listen for profile update events (triggered after transcription)
  useEffect(() => {
    const handleProfileUpdate = () => {
      console.log("Profile update triggered by transcription event");
      apiService.user.getUserPersona()
        .then(response => {
          const updatedData = response.data;
          setUserPersona(currentPersona => {
            if (JSON.stringify(updatedData) !== JSON.stringify(currentPersona)) {
              console.log("Profile updated via event trigger");
              if (!isEditing) { 
                setEditablePersona(JSON.parse(JSON.stringify(updatedData))); 
              }
              setLastUpdated(new Date());
              return updatedData;
            }
        ...
        .catch(error => console.error("Error updating profile via event:", error));
    };

    window.addEventListener('profileUpdateNeeded', handleProfileUpdate);
    return () => window.removeEventListener('profileUpdateNeeded', handleProfileUpdate);
  }, [isEditing]);

  // Smart polling - only when user is actively chatting and profile can be updated
  useEffect(() => {
    if (isEditing || !userPersona || saveStatus === 'auto-saving') return; // Avoid polling during auto-save

    const intervalId = setInterval(() => {
      // Only poll if page is visible (user is actively using the app)
      if (document.visibilityState !== 'visible') return;
      
      console.log("Backup polling for user persona updates...");
      apiService.user.getUserPersona()
        .then(response => {
    ...
    }, 60000); // Reduced to 60 seconds since we have event-driven updates

    return () => clearInterval(intervalId);
  }, [isEditing, userPersona, saveStatus]);
