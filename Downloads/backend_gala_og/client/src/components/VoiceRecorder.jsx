  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mediaRecorder.current.onstop = async () => {
        ...
        try {
          const response = await apiService.chat.sendTranscription(formData);
          const data = response.data;

          if (data?.messages) {
            addMessage(data.messages); // Add transcribed messages to chat
            
            // TRIGGER IMMEDIATE PROFILE UPDATE after transcription
            // This reduces the need for frequent polling since profile updates during conversation
            window.dispatchEvent(new CustomEvent('profileUpdateNeeded'));
          }
        } catch (error) {
    ...
    <div className="relative">
      ...
      <button
        ...
        className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${
          isRecording
            ? "bg-purple-200 hover:bg-purple-300 scale-110"
